...
import { authLogin } from "@/lib/apis/public/auth";
...
export default function SignIn() {
  ...
  const loginUser = async (data: SignInModel) => {
    setLoading(true);
    try {
      ...
      if (response?.ok) {
        user.setLoggedIn(responseData?.access);
  ...
  return (
    <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark h-screen">
      <div className="flex flex-wrap items-center h-full">
        <div className="hidden relative w-full xl:block xl:w-1/2 h-full">
          <div className="py-17.5 px-26 text-center">
            <h1 className="2xl:px-20 text-2xl font-bold absolute top-1/2 left-1/2 -translate-x-1/2">
              FITCONNECT {}
            </h1>
          </div>
        </div>
